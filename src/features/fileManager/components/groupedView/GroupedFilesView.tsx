...
import { Box, Grid, TextField, MenuItem, Typography, IconButton, Pagination } from "@mui/material";
import { 
  ArrowBack,
  Image as ImageIcon,
...
export interface GroupedFilesViewProps {
  ...
  onGroupChange?: (group: {name: string, icon: React.ReactNode} | null) => void;
  onBackToGroups?: React.MutableRefObject<(() => void) | null>;
}

type SortKey = "name" | "size" | "updated";

const GroupedFilesView: React.FC<GroupedFilesViewProps> = ({ files, onOpenFile, onDeleteFile, onOpenGroup, onGroupChange, onBackToGroups }) => {
  ...
  const mappedFiles = useMemo(() => {
    return files.map(file => ({
      id: file.id.toString(),
      name: (file as any).fileName || (file as any).storedFileName || `File ${file.id}`,
  ...
  const getGroupIcon = (group: string) => {
    switch (group) {
      ...
      case "Spreadsheets": return <SpreadsheetIcon />;
      case "Presentations": return <PresentationIcon />;
      case "PDFs": return <PdfIcon />;
      case "Audio": return <AudioIcon />;
      case "Video": return <VideoIcon />;
      case "Archives": return <ArchiveIcon />;
  ...
  const getGroupColor = (group: string) => {
    switch (group) {
      case "Images": return "#4CAF50";
      case "Documents": return "#2196F3";
      case "Spreadsheets": return "#FF9800";
      case "Presentations": return "#E91E63";
      case "PDFs": return "#F44336";
      case "Audio": return "#9C27B0";
      case "Video": return "#FF5722";
      case "Archives": return "#795548";
  ...
  if (selectedGroup && (groups as any)[selectedGroup]) {
    ...
    const filteredGroupFiles = groupQuery.trim() 
      ? allGroupFiles.filter((f: any) => (f.name || "").toLowerCase().includes(groupQuery.trim().toLowerCase()))
      : allGroupFiles;
    
    const totalPages = Math.ceil(filteredGroupFiles.length / pageSize);
    const startIndex = (page - 1) * pageSize;
    const paginatedFiles = filteredGroupFiles.slice(startIndex, startIndex + pageSize);
      ...
      <Box sx={{ p: 2, display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
          {paginatedFiles.map((f: any) => (
              <FileTile
                name={f.name}
                subtitle={[f.updatedAt ? `Updated ${formatDistanceToNow(new Date(f.updatedAt))} ago` : undefined].filter(Boolean).join(" • ")}
                onOpen={() => {
                  const originalFile = files.find(file => file.id.toString() === f.id);
  ...
  return (
    <Box sx={{ p: 2, display: "flex", flexDirection: "column", gap: 2 }}>
      <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
        <TextField size="small" label="Search all files" value={query} onChange={(e) => setQuery(e.target.value)} sx={{ width: 300 }} />
